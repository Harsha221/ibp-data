...
<sec:access url="/association/edit/**"> <% access = true %> </sec:access>
<sec:access url="/association/approve/**"> <% access = true %> </sec:access>
<div class="table-responsive table_data">
    <table class="table table-striped" id="categoryListTable">
        ...
        <tbody>
        <g:each in="${associationList}" var="data">
            <tr>
                <td>${data?.name}</td>
                <td>${data?.active ? 'Activated' : 'Deactivated'}</td>
                <td><g:if test="${data?.headerUrl}" ><img height="50px" width="100px" src="${data?.headerUrl}"/></g:if></td>
                <td>${data?.redirectLink}</td>
                <td><g:if test="${data?.logoUrl}" ><img height="50px" width="100px" src="${data?.logoUrl}"/></g:if></td>
                ...
                <td>
                    <div class="input-group-btn" >
                        ...
                        <ul class="dropdown-menu pull-right" style="min-width: 100px !important;">
                            ...
                            <sec:access url="/association/approve/**">
        ...
        <g:if test="${!associationList}">
            <tr>
                <td colspan="7" class="text-center"><strong>No Association found</strong></td>
            </tr>
        </g:if>
        </tbody>
    </table>
...
<div class="page_data">
    <div>
        <g:if test="${associationCount > 0}">
            Showing ${params?.offset ? Integer.parseInt(params?.offset) + 1 : 1} to ${params?.offset ? associationCount > (params?.max + Integer.parseInt(params?.offset)) ? params?.max + Integer.parseInt(params?.offset) : associationCount : associationCount < params?.max ? associationCount : params?.max} of ${associationCount} entries
        </g:if>
    </div>
    <div>
        <bootstrap:remotePaginate
                class="pull-right"
                total="${associationCount ?: 0}"
...
<script type="text/javascript">
    $('a.status-link').click(function () {
        ...
        if($(this).html().trim() === 'Deactivated') {
            $(".form-group-search").html('Are you sure you want to Deactivate Association ?')
        } else {
            $(".form-group-search").html('Are you sure you want to Activate Association ?')
        }
    })
</script>