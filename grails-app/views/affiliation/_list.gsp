...
<sec:access url="/affiliation/edit/**"> <% access = true %> </sec:access>
<sec:access url="/affiliation/approve/**"> <% access = true %> </sec:access>
<div class="table-responsive table_data">
    <table class="table table-striped" id="categoryListTable">
        ...
        <g:each in="${affiliationList}" var="data">
            <tr>
                <td><g:link action="affiliationProduct" params="${[categoryId: data?.id]}">${data?.name}</g:link></td>
                <td>${data?.active ? 'Activated' : 'Deactivated'}</td>
                <td style="width: 25px !important;">${data?.websiteLink}</td>
                    ...
                    <td>
                        <div class="input-group-btn" >
                            ...
                            <ul class="dropdown-menu pull-right" style="min-width: 100px !important;">
                                ...
                                <sec:access url="/affiliation/approve/**">
...
<script type="text/javascript">
    $('a.status-link').click(function () {
        ...
        if($(this).html().trim() === 'Deactivated') {
            $(".form-group-search").html('Are you sure you want to Deactivate Affiliation ?')
        } else {
            $(".form-group-search").html('Are you sure you want to Activate Affiliation ?')
        }
    })
</script>