...
<sec:access url="/logo/edit/**"> <% access = true %> </sec:access>
<sec:access url="/logo/approve/**"> <% access = true %> </sec:access>
<div class="table-responsive table_data">
    <table class="table table-striped" id="vendorsListTable">
        ...
        <g:if test="${!logoList}">
            <tr>
                <td colspan="8" class="text-center"><strong>No Logo found</strong></td>
            </tr>
        </g:if>
        </tbody>
    </table>
...
<div class="page_data">
    <div>
        <g:if test="${logoCount > 0}">
            Showing ${params?.offset ? Integer.parseInt(params?.offset) + 1 : 1} to ${params?.offset ? logoCount > (params?.max + Integer.parseInt(params?.offset)) ? params?.max + Integer.parseInt(params?.offset) :logoCount : logoCount < params?.max ? logoCount : params?.max} of ${logoCount} entries
        </g:if>
    </div>
    <div>
        <bootstrap:remotePaginate
                ...
                onLoading="showLoader('logo-list-div')"
                params="${params}"
        />
    </div>
</div>
                    ...
                    <div class="row m-t-10">
                        <div class="col-md-12">
                            ...
                            <div class="form-group-search">
                                Are you sure you want to Deactivate Logo ?
                            </div>
                        </div>
                    </div>
                </div>
...
<script type="text/javascript">
    $('a.status-link').click(function () {
        ...
        if($(this).html().trim() === 'Deactivated') {
            $(".form-group-search").html('Are you sure you want to Deactivate Logo ?')
        } else {
            $(".form-group-search").html('Are you sure you want to Activate Logo ?')
        }
    })
</script>