...
<sec:access url="/blogCategory/edit/**"> <% access = true %> </sec:access>
<sec:access url="/blogCategory/approve/**"> <% access = true %> </sec:access>
<div class="table-responsive table_data">
    <table class="table table-striped" id="vendorsListTable">
        ...
        <tbody>
        <g:each in="${blogCategoryList}" var="data">
            <tr>
                <td>${data?.name}</td>
                    ...
                    <td>
                        <div class="input-group-btn" >
                            ...
                            <ul class="dropdown-menu pull-right" style="min-width: 100px !important;">
                                ...
                                <sec:access url="/blogCategory/approve/**">
        ...
        <g:if test="${!blogCategoryList}">
            <tr>
                <td colspan="8" class="text-center"><strong>No Blog Category found</strong></td>
            </tr>
        </g:if>
        </tbody>
    </table>
...
<div class="page_data">
    <div>
        <g:if test="${blogCategoryCount > 0}">
            Showing ${params?.offset ? Integer.parseInt(params?.offset) + 1 : 1} to ${params?.offset ? blogCategoryCount > (params?.max + Integer.parseInt(params?.offset)) ? params?.max + Integer.parseInt(params?.offset) : blogCategoryCount : blogCategoryCount < params?.max ? blogCategoryCount : params?.max} of ${blogCategoryCount} entries
        </g:if>
    </div>
    <div>
        <bootstrap:remotePaginate
                class="pull-right"
                total="${blogCategoryCount ?: 0}"
                update="blogCategory-list-div"
                onLoading="showLoader('blogCategory-list-div')"
                params="${params}"
        />
    </div>
</div>
                    ...
                    <div class="row m-t-10">
                        <div class="col-md-12">
                            ...
                            <div class="form-group-search">
                                Are you sure you want to Deactivate Blog Category ?
                            </div>
                        </div>
                    </div>
                </div>
...
<script type="text/javascript">
    $('a.status-link').click(function () {
        ...
        if($(this).html().trim() === 'Deactivated') {
            $(".form-group-search").html('Are you sure you want to Deactivate BlogCategory ?')
        } else {
            $(".form-group-search").html('Are you sure you want to Activate BlogCategory ?')
        }
    })
</script>