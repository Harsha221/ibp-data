...
<sec:access url="/homeBanners/edit/**"> <% access = true %> </sec:access>
<sec:access url="/homeBanners/approve/**"> <% access = true %> </sec:access>
<div class="table-responsive">
    <table class="table table-striped" id="vendorsListTable">
        ...
        <tbody>
        <g:each in="${homeBannersList}" var="data">
            <tr>
                <td>${data?.title}</td>
                    ...
                    <td>
                        <div class="input-group-btn" >
                            ...
                            <ul class="dropdown-menu pull-right" style="min-width: 100px !important;">
                                ...
                                <sec:access url="/homeBanners/approve/**">
        ...
        <g:if test="${!homeBannersList}">
            <tr>
                <td colspan="8" class="text-center"><strong>No Banner found</strong></td>
            </tr>
        </g:if>
        </tbody>
    </table>
    <g:if test="${homeBannersCount > 0}">
        Showing ${params?.offset ? Integer.parseInt(params?.offset) + 1 : 1} to ${params?.offset ? homeBannersCount > (params?.max + Integer.parseInt(params?.offset)) ? params?.max + Integer.parseInt(params?.offset) : homeBannersCount : homeBannersCount < params?.max ? homeBannersCount : params?.max} of ${homeBannersCount} entries
    </g:if>
    <bootstrap:remotePaginate
            class="pull-right"
            total="${homeBannersCount ?: 0}"
            update="homeBanners-list-div"
            onLoading="showLoader('homeBanners-list-div')"
            params="${params}"
    />
</div>
                    ...
                    <div class="row m-t-10">
                        <div class="col-md-12">
                            ...
                            <div class="form-group">
                                Are you sure you want to Deactivate homeBanners ?
                            </div>
                        </div>
                    </div>
                </div>
...
<script type="text/javascript">
    $('a.status-link').click(function () {
        ...
        if($(this).html().trim() === 'Deactivated') {
            $(".form-group").html('Are you sure you want to Deactivate HomeBanners ?')
        } else {
            $(".form-group").html('Are you sure you want to Activate HomeBanners ?')
        }
    })
</script>