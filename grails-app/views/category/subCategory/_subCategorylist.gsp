...
<sec:access url="/category/editSubCategory/**"> <% access = true %> </sec:access>
<sec:access url="/category/approveSubCategory/**"> <% access = true %> </sec:access>
<div class="table-responsive table_data">
    <table class="table table-striped" id="categoryListTable">
        ...
        <g:each in="${categoryList}" var="data">
            <tr>
                <td><g:link controller="product" action="index" params="${[subCategoryId: data?.id]}">${data?.name}</g:link></td>
                ...
                <td>
                    <div class="input-group-btn" >
                        ...
                        <ul class="dropdown-menu pull-right" style="min-width: 100px !important;">
                            <li>
                                <sec:link action="editSubCategory"  id="${data?.id}" data-job-id="${data?.id}" class="status-link">Edit</sec:link>
                            </li>
                            <sec:access url="/category/approveSubCategory/**">
        ...
        <g:if test="${!categoryList}">
            <tr>
                <td colspan="7" class="text-center"><strong>No SubCategory found</strong></td>
            </tr>
        </g:if>
        </tbody>
    </table>
...
<div class="page_data">
    ...
    <div><bootstrap:remotePaginate
            class="pull-right"
            total="${categoryCount ?: 0}"
            update="sub-category-list-div"
            onLoading="showLoader('sub-category-list-div')"
            params="${params}"
    />
    </div>
</div>
...
<script type="text/javascript">
    ...
    $('a.status-link').click(function () {
        ...
        if($(this).html().trim() === 'Deactivated') {
            $(".form-group-search").html('Are you sure you want to Deactivate SubCategory ?')
        } else {
            $(".form-group-search").html('Are you sure you want to Activate SubCategory ?')
        }
    })
</script>
